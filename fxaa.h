...
#define FXAA_BLOCK_W 16
#define FXAA_BLOCK_H 16
...
__global__ void fxaa_pass_shared(const vec3* src, vec3* dst, int w, int h) {
    int i = threadIdx.x + blockIdx.x * blockDim.x;
    int j = threadIdx.y + blockIdx.y * blockDim.y;

    //local thread coords inside the block
    int tx = threadIdx.x;
    int ty = threadIdx.y;

    // shared memory, block size+2 to get neighboring pixels on the edge
    __shared__ vec3 s_color[FXAA_BLOCK_H+2][FXAA_BLOCK_W+2];

    // local coordinates in shared memory shift by 1 for halo
    int lx = tx +1;
    int ly = ty+1;
    ...
    // thread loads its pixel
    s_color[ly][lx] = fetch(i,j);

    // threads loading the pixels W,E,N,S
    if (tx == 0)
        s_color[ly][0]= fetch(i - 1, j);     
    if (tx == blockDim.x - 1)
        s_color[ly][lx + 1]= fetch(i + 1, j);     
    if (ty == 0)
        s_color[0][lx] = fetch(i, j - 1);     
    if (ty == blockDim.y - 1)
        s_color[ly + 1][lx]= fetch(i, j + 1);     

    // other threads load 4 corners
    if (tx == 0 && ty == 0)                                 
        s_color[0][0]= fetch(i - 1, j - 1); 
    if (tx == blockDim.x - 1 && ty == 0)
        s_color[0][lx + 1]= fetch(i + 1, j - 1); 
    if (tx == 0 && ty == blockDim.y - 1)
        s_color[ly + 1][0]= fetch(i - 1, j + 1);
    if (tx == blockDim.x - 1 && ty == blockDim.y - 1)
        s_color[ly + 1][lx + 1] = fetch(i + 1, j + 1);

    __syncthreads();
    if (i >= w || j >= h) return;

    vec3 C  = s_color[ly][lx];
    vec3 N  = s_color[ly-1][lx];
    vec3 S  = s_color[ly+1][lx];
    vec3 W  = s_color[ly][lx-1];
    vec3 E  = s_color[ly][lx+1];
    ...
    //diagonal neighbors for a better gradient estimate
    vec3 NW = s_color[ly-1][lx-1];
    vec3 NE = s_color[ly-1][lx+1];
    vec3 SW = s_color[ly+1][lx-1];
    vec3 SE = s_color[ly+1][lx+1];
